...
from minimal_robot.nav_math import (
    lookahead_point, normalize_angle, yaw_from_quaternion)
    ...
    def _control_step(self) -> None:
        ...
        if self._check_pos is None or now - self._check_t > 3.0:
            if (self._check_pos is not None and math.hypot(
                ...
                to_target = normalize_angle(math.atan2(ty - y, tx - x) - yaw)
        ...
        lax, lay = lookahead_point(self.prev_point, (tx, ty), x, y, lookahead)
        heading_error = normalize_angle(math.atan2(lay - y, lax - x) - yaw)
